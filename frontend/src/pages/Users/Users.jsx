...
export default function Users() {
  ...
  useEffect(() => {
    axios
      .get("https://demo-repo23.herokuapp.com/users/list")
      .then((response) => {
        if (response.data) {
  ...
  const handleCreateUser = () => {
    axios
      .post(`https://demo-repo23.herokuapp.com/users/create/${newUserName}`)
      .then(() => {
      ...
      <div className="rooms-list">
        {users ? (
          users.map((user, index) => (
            <div className="user-item" key={`${user.userName}-${index}`}>
              <p>{user.userName}</p>
              <p>{index}</p>
            </div>
          ))
